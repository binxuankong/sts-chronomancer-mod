...
public class ArcaneBarrageAction extends AbstractGameAction {
    private DamageInfo info;

    public ArcaneBarrageAction(AbstractCreature target, DamageInfo info) {
        this.info = info;
    ...
    public void update() {
        if (this.duration == 0.01F && this.target != null && this.target.currentHealth > 0) {
            ...
            AbstractDungeon.effectList.add(new FlashAtkImgEffect(this.target.hb.cX +
                    ...
                    this.target.hb.cY + MathUtils.random(-100.0F * Settings.scale, 100.0F * Settings.scale),
                    this.attackEffect));
        }
